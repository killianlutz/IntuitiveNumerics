...
nframe = 800
npoints = 40
xygrid_lims = (-5.0, 5.0, -5.0, 5.0)#(-1.0, 2.0, -1.0, 2.0)
params = (; nframe, npoints, xygrid_lims)

" gradient step "
fig, step_forward = setup_gradient(params...; step_size=0.0005, objective_type="multimodal")
# rosenbrock : 0.01, 0.005, 0.004, 0.001 ; 1500 frames ; (0.5, 1.2, 0.5, 1.2) or (-1.5, 1.5, -1.5, 1.5)
" gradient step in 3D "
fig, step_forward = setup_gradient3d(params...; step_size=0.05, objective_type="gaussian", is_dynamic=false)

" optimal step gradient "
fig, step_forward = setup_linesearch_gradient(params...; step_size=0.1, objective_type="multimodal")

" projected gradient "
fig, step_forward = setup_projgradient(params...; step_size=0.01, objective_type="gaussian", constraint_type="ball")

" uzawa/arrow-hurwicz "
A = [1.0 0.05; 0.05 1.0] # positive definite ?
b = A*[1.5, 1.2]
fig, step_forward = setup_primaldual3d(params...; step_size=0.05, algorithm="uzawa", A, b)
# step size 0.05 for uzawa/arrowhurwicz and limits (-1.0, 2.0, -1.0, 2.0)

" augmented lagrangian "
A = [1.0 0.05; 0.05 1.0] # positive definite ?
b = A*[1.5, 1.2]
B = [1.0 0; 0 4.0] # try with anisotropic axes
...
fig, step_forward = setup_penalized_gradient3d(params...; step_size=0.05, objective_type="gaussian", ϵ=5.0, is_dynamic=false)
